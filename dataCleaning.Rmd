...
#right now the "select" deletes all the other variables that we need which is why I put this in # for now
#dat %>%
...
```{r}
dat %>%
  select(dyadID, partnum, gender, religion, birthday, sexualor, race, relation_length, RecordedDate, indinc, wfh, starts_with("housework_"), starts_with("grbs_"))
head(dat)
```
...
head(dat)
#check whether avg is calculated with missing values subtracted 
#report how averages are calculated (remove missing values)
```
...
overall_data <- dat_reverse %>%
  select(dyadID, partnum, gender, religion, avg_housework_male, avg_housework_female, avg_grbs, housework_fair, housework_satisfied)

show(overall_data$partnum)
```
...
#removing the ones that have no partner 
remove<-c(13, 132, 173, 222, 267, 274, 331)
overall_data_clean<-overall_data[-remove,]

#create partnum with gender var
...
tempA <- overall_data %>%
  mutate(genderE = gender, partnum2 = 1,
   ...
   gather(variable, value, avg_housework_male:genderE) %>%
...
tempB <- overall_data %>%
   mutate(genderE = gender, partnum = 2,
          gender = ifelse(gender == "Man", "P", "A")) %>%
   gather(variable, value, Religion:housework_satisfied)%>%
   unite(var_ap, variable, gender) %>%
   spread(var_ap, value)

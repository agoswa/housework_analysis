...
dat <- dat %>%
  select(dyadID, partnum, starts_with("housework_"), starts_with("grbs_"), religion)
head(dat)
```
...
```{r}
# adding an average housework column
dat$avg_housework = rowMeans(dat[,c(3:16)])

# adding an average grbs column
dat$avg_grbs = rowMeans(dat[,c(19:38)])
...
overall_data <- dat %>%
  select(dyadID, avg_housework, avg_grbs, housework_fair, housework_satisfied, religion)
...
```{r}
# Correlation devided by housework type
corr.test(select(dat, housework_1, housework_2, housework_3, housework_4, 
...
```{r}
# average devided by housework type
dat
dat$avg_housework_female = rowMeans(dat[,c(3:6, 9:11, 14:16)])
dat$avg_housework_male = rowMeans(dat[,c(7:8, 12:13)])
dat
```
...
```
We should check for reversed items!!

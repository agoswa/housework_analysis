...
```
Originally, `r n2` individuals in a partnership living in the United States of America participated in the study. In our analysis, we excluded all non-heterosexual couples and participants that did not have any partner variables available. In the end, N=`n` couples have been included in the analysis. Men and women from the final sample of `n`  adult couples were `r womenmean` (SD = `r womensd`, range = `r womenrangelow`-`r womenrangehigh`) and `r menmean` (SD = `r mensd`, range = `r menrangelow`-`r menrangehigh`) years old, respectively. 

The relationship were between `r lengthrangelow` and `r lengthrangehigh` years long, with an average of `r lengthrange` years (SD = `r lengthsd`). The average income was `r menincmean` USD (SD = `r menincsd`) for men and `r womenincmean` USD (SD = `r womenincsd`) for women. `r Wpercnohome` % of the women and `r Mpercnohome` % of the men were not working from home, although it must be noted that `r MpercnohomeNA` % of the men and `r WpercnohomeNA` % of the women did not answer the question.

`r Christian` % of the sample identified as Christian, `Athiest` % as Athiest, `r Agnostic` % as Agnostic, `r Judaism` % as Jewish, `r Hinduism` % as Hindu and `r Islam` % as Muslim. `r other_comb` % identified had a religious orientation apart from the mentioned ones and `r noanswer` % prefered not to answer this question.
...
Since the p-value is 0.83 which is greater than 0.05, we can fail to reject the null hypothesis. This means that there is not sufficient evidence in the data to say that there is a true difference in both the means.

...
```{r, include = TRUE,wife_plot, fig.cap = "(ref:my-figure-caption1)"}
wife_plot<- ggplot(wives,aes(
                x = avg_housework_female_A,
  ...
  labs(x = "Housework distribution", y = "Satisfaction", title = "Relationship of wive's housework distribution and gender role beliefs", caption = "Caption should start on the left")
...
```{r,include = TRUE, wife_plot2, fig.cap = "(ref:my-figure-caption2)"}
...
apim_stp1 <- gls(avg_grbs_A ~ genderE_A + housework_satisfied_A:genderE_A + housework_satisfied_P:genderE_A - 1,
                 data = dat_dyad,
                 correlation = corCompSymm(form=~1|dyadID), 
                 weights = varIdent(form=~1|genderE_A), 
                 na.action = na.omit)
...
apim_stp3 <- gls(housework_satisfied_A ~ genderE_A + avg_grbs_A:genderE_A + avg_grbs_P:genderE_A 
                 + avg_grbs_A:genderE_A + avg_grbs_P:genderE_A - 1,
                 data = dat_dyad,
                 correlation = corCompSymm(form=~1|dyadID), 
                 weights = varIdent(form=~1|genderE_A), 
                 na.action = na.omit)
